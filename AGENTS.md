...
- **`services/`** — Business logic + external API clients.
  ...
  - `manual_import/` — #122 manual import for existing libraries (Part A: preview). `walk.rs` (walkdir over a user folder: hidden / junk-dir / symlink toggles, Ryokan's `media_root` + recycle bin always excluded, 50k-file cap), `parse.rs` (series title via `library_link::extract_anime_title` with a parent-folder fallback that climbs past `Season 01`-style dirs; folder names skip anitomy unless release tokens are present because anitomy reads a trailing `100` as an episode; episode/season via `media::parse_episode_number`), `matching.rs` (query building, ranking of AL `SEARCH_MATCH` results by title similarity + year + file-count-vs-format + season marker, low-confidence flag), `preview.rs` (pure projection of per-file outcomes: Import / Replace via `source::is_valid_upgrade` / Already-have / Pinned / Downloading / No-episode; folder-collision suffixing), `session.rs` (the `AppState.import_sessions` store). `mod.rs` runs the background job under a `ProgressRegistry` handle keyed by the session id (walk → parse → group → match at `MATCH_CONCURRENCY = 4` → resolve existing series **by AL/MAL id, never title**). **Nothing here mutates the library**; the import step (Part B) builds on the session. Matched-existing tags snapshot into the session at resolve time so re-renders don't re-query.
...
- `import_sessions: ImportSessionStore = Arc<Mutex<HashMap<String, ImportSession>>>` — #122 manual-import preview sessions keyed by the opaque id in `/library/import?session=<id>`. Same shape as `interactive_search_cache`: idle sessions evict on the next access (2h TTL, 8-session cap). Holds walk results plus the user's match / file decisions between wizard steps; handlers clone a session out rather than hold the lock across an await. Nothing in it is persisted.
