...
The `sonarr_compat` / `radarr_compat` routers are merged **outside** the cookie-auth layer and use `arr_auth::check_api_key`. See `src/handlers/sonarr_compat/CLAUDE.md` if it exists, otherwise `arr_auth.rs` directly.
