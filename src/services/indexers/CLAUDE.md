...
- **Per-indexer rate limits live inside Prowlarr/Jackett**, not the indexer itself. They surface as `429 Retry-After`. The client surfaces `retry_after` seconds in the error message; **no process-wide cooldown is applied yet** — every fan-out re-issues whether the previous burst hit a 429 or not. The cooldown table mirroring `services::anilist::rate_limit` is tracked as a follow-up; until it lands, a 429-storm just logs at debug and keeps trying.
- **`tvsearch` with `cat=5070&q=<title>` is the right anime path.** `season`/`ep` params don't translate cleanly because anime trackers key on absolute episode numbers in titles, so `SearchQuery` deliberately omits them.
...
Each indexer row carries an optional `download_client_id`. Resolution chain at grab time (`AppState::client_for_indexer_with_id`): pin → per-protocol default (torznab → torrent default; newznab → usenet default) → torrent fallback. Deep dive in root CLAUDE.md under "Download-client routing."
