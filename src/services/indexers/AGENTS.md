...
- **`tvsearch` uses each row's configured category ids and `q=<title>`.** Existing rows default to anime category `5070`; dedicated feeds may use another category such as adult content `6000`. `season`/`ep` params don't translate cleanly because anime trackers key on absolute episode numbers in titles, so `SearchQuery` deliberately omits them.
