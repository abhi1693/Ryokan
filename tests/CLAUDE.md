...
Each `services/download_client/*/mod.rs` impl ships a `#[ignore]`d `live_smoke` test that exercises the full trait surface against a real client on localhost. Run with `--ignored` *and* `RYOKAN_{QBIT,DELUGE,TRANSMISSION,RTORRENT}_E2E=1` set. CI never runs them. See `src/services/download_client/CLAUDE.md` for per-client setup.
