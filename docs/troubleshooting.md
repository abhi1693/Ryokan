...
- **Settings → Connections → Test connection** for every download client and indexer. Connection tests catch most config issues at config time rather than at grab time.
...
**Fix**: click Test connection on the SAB row in Settings → Connections → Downloads. The auto-create path will create the missing category and re-tag the just-added job. After that, future grabs land correctly.
...
- **Post-processing is disabled** in Settings → Media. Ryokan correctly leaves the file at the download client's path; the row shouldn't be showing "Importing…" in this state. If it is, force-refresh the page (Ctrl+Shift+R) — there's a known race where the per-row state can lag the global toggle.
...
- **Profile mismatch**: your active quality profile doesn't accept any of the released qualities. Try widening the profile (Settings → Quality) or use the Interactive Search button on the episode for a one-off relaxed search.
- **Custom Format scoring threshold**: if `custom_format_minimum_score` is set (Settings → Quality), releases scoring below it are silently dropped from auto-search candidates. They still show up in interactive search.
