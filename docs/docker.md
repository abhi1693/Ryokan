...
| `RYOKAN_POST_PROCESSING_VERIFY_CONCURRENCY` | `1` | Read-only workers used to byte-verify an existing destination before adopting it as a durable import checkpoint. Values are clamped to `1` through `8`; library and database mutations remain serialized. |
| `RYOKAN_POST_PROCESSING_MAX_FILES_PER_GRAB` | unset | Optional per-pass file budget for each completed grab. Set it to `1` to let every pending grab make progress before a large batch receives another turn. Durable per-file checkpoints preserve completed work between passes. |
