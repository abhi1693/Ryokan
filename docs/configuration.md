...
- **RSS sync interval**: how often the background poller pulls from configured RSS sources. Default 15 minutes; minimum 1, maximum 60. Some feeds (NZBGeek, etc.) tend to rate-limit at five-minute polling, which is why the default isn't lower.
- **External-account sync interval**: how often AniList / MAL watch-list sync runs. Minimum 15 minutes (You can't go lower no matter what you put in the field).
