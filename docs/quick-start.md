...
**Settings → Indexers → Add indexer**. Paste the URL Prowlarr or Jackett gave you (it ends in `/api`), the API key, and pick a name. The category default `5070` covers anime; comma-separate ids if the same row needs more than one category.
